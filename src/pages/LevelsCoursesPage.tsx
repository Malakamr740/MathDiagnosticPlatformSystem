...
export default function LevelsCoursesPage() {
  ...
  // Soft-delete level with confirmation
  async function handleDeleteLevel(lvl: LevelRow) {
    const confirmMessage = `Are you sure you want to deactivate level "${lvl.name}"? Past reports referencing this tier will remain preserved.`
    if (!window.confirm(confirmMessage)) return

    // Optimistic UI update
    const previousLevels = [...levels]
    setLevels((prev) => prev.map((l) => (l.id === lvl.id ? { ...l, is_active: false } : l)))
    ...
    try {
      ...
      addToast('error', err?.message || 'Failed to deactivate level.')
    }
  }
  ...
  // Toggle Category Tag on a Level with Optimistic UI & Rollback
  async function handleToggleCategory(levelId: string, categoryId: string) {
    ...
    // Optimistic update
    setLevelCategoryMap((prev) => ({ ...prev, [levelId]: nextCats }))
    ...
    try {
      ...
      // Rollback
      setLevelCategoryMap((prev) => ({ ...prev, [levelId]: currentCats }))
      addToast('error', 'Could not update category link. Rolled back.')
    }
  }
      ...
      actions={
        <div className="flex items-center gap-2">
          <button
            ...
            className="btn-primary rounded-lg bg-blue-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xs hover:bg-blue-700 transition"
          >
            ...
            <form onSubmit={handleSaveCourse} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  style={{ backgroundColor: '#2563eb', color: '#ffffff' }}
                  className="btn-primary px-4 py-1.5 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition shadow-xs disabled:opacity-50"
                >
